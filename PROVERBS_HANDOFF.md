...
- `web/` 提供 20×20、四个暂定大区的可玩页面；公开题面不携带隐藏答案，浏览器端提示器与 Python 求解规则保持一致，并可区分关卡级“含高级推理”和当前步骤级“高级推理”。
- `npm test` 已覆盖区域裁剪、求解器状态、MiniZinc 第二解判定和真实关卡生成；`npm run generate` 可重新生成 `web/data/demo-level.json`。

当前实现是规则闭环原型，不声称复现 Proverbs 的原始素材、正式区域布局或未公开的高级 solver 规则；本项目的“高级推理”目前特指确定性的集合包含差集规则。
...
7. 在现有高级差集推导基础上继续扩展规则，再决定全局难度分层和内容/答案图案对齐。
