...
function surface(kind){
 ...
 for(let y=0;y<512;y++)for(let x=0;x<512;x++){
  ...
  else if(kind==='stone'){const vein=Math.pow(Math.abs(Math.sin(x*.021+y*.018+Math.sin(x*.035)*1.1)),18),v=48+n*4+vein*8;rgb=[v*.77,v*.9,v*.86];}
  else if(kind==='cloth'){const weave=((x%3===0?1:0)+(y%3===0?1:0))*5,v=45+weave+n*4;rgb=[v*1.35,v*.32,v*.41];}
...
export async function createMaterials(){
 ...
 const [woodMap,woodNormal,woodRough]=await Promise.all(['wood-color.jpg','wood-normal.jpg','wood-roughness.jpg'].map(name=>loader.loadAsync('./assets/'+name)));
 ...
 return {
  wood:physical({map:woodMap,normalMap:woodNormal,normalScale:new T.Vector2(.55,.55),roughnessMap:woodRough,color:'#ffffff',roughness:.9,metalness:0,clearcoat:.12,clearcoatRoughness:.32}),
  stone:physical({map:stoneMap,roughness:.47,metalness:0,bumpMap:stoneMap,bumpScale:.018}),
  cloth:physical({map:clothMap,roughness:.94,sheen:.85,sheenColor:new T.Color('#922c34'),bumpMap:clothMap,bumpScale:.028}),
  gold:physical({color:'#e2bb73',metalness:1,roughness:.24,roughnessMap:micro,bumpMap:micro,bumpScale:.006}),
  bronze:physical({color:'#987344',metalness:.93,roughness:.37,roughnessMap:micro}),
  ivory:physical({color:'#e7e1cd',metalness:0,roughness:.17,clearcoat:1,clearcoatRoughness:.12}),
  dark:physical({color:'#152a24',roughness:.85,sheen:.7,sheenColor:new T.Color('#426251'),bumpMap:clothMap,bumpScale:.025}),
  glass:physical({color:'#eee8d7',metalness:0,roughness:.06,transmission:.94,thickness:.08,ior:1.46}),
  paper:physical({roughness:.94,bumpMap:micro,bumpScale:.008}),
 };
}
