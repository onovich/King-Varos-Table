import {createSmoothCamera} from './smooth-camera.mjs';
import * as T from './vendor/three.module.min.js';
import {createScene} from './scene.mjs?v=12';
import {regions,MAP,regionAt} from './map-data.mjs?v=12';
import {createMotion,isClick} from './motion.mjs';
import {cameraDestination,overviewDistance as fitDistance} from './camera.mjs?v=12';
import {createBoard,cellAt,boardOpacity,BOARD} from './board.mjs?v=12';
...
const stage=await createScene(renderer,{blenderTableware:params.get('craft')!=='classic',bakedLighting:params.get('lighting')!=='legacy'}),camera=new T.PerspectiveCamera(35,1,.1,100),target=new T.Vector3(),board=createBoard(stage.scene);
...
let fireOverride=null;const fireToggle=document.getElementById('fireMotion');
function updateFireLabel(){const enabled=fireOverride??!reduced.matches;fireToggle.setAttribute('aria-pressed',String(enabled));fireToggle.textContent='烛火动态：'+(enabled?'开':'关');}
fireToggle.onclick=()=>{fireOverride=!(fireOverride??!reduced.matches);updateFireLabel();};reduced.addEventListener('change',updateFireLabel);updateFireLabel();
...
let smooth=null,lastFrame=0;
function apply(p){if(!smooth){smooth=createSmoothCamera(p);camera.position.fromArray(p.position);target.fromArray(p.target);camera.lookAt(target);camera.updateMatrixWorld();}else smooth.set(p);}
...
function pan(dx,dz){const p=smooth.target,x=T.MathUtils.clamp(p.target[0]+dx,-MAP.width*.55,MAP.width*.55),z=T.MathUtils.clamp(p.target[2]+dz,-MAP.height*.55,MAP.height*.55);p.position[0]+=x-p.target[0];p.position[2]+=z-p.target[2];p.target[0]=x;p.target[2]=z;apply(p);}
function zoom(factor){const p=smooth.target,at=new T.Vector3(...p.target),offset=new T.Vector3(...p.position).sub(at),distance=T.MathUtils.clamp(offset.length()*factor,5,overviewDistance()*1.25);offset.setLength(distance);p.position=at.add(offset).toArray();apply(p);}
function takeOver(){if(motion.active){motion.cancel();smooth.set(pose());}}
canvas.addEventListener('pointerdown',e=>{if(view==='banquet')return;takeOver();canvas.setPointerCapture(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===1)gesture={x:e.clientX,y:e.clientY,max:0,multiple:false};else gesture.multiple=true;$('hoverLabel').hidden=true;});
...
canvas.addEventListener('wheel',e=>{if(view==='banquet')return;e.preventDefault();takeOver();zoom(Math.exp(T.MathUtils.clamp(e.deltaY,-150,150)*.0015));},{passive:false});
...
function animate(time){if(document.hidden)return;const p=motion.sample(time);if(p)apply(p);const shown=smooth.step(lastFrame?(time-lastFrame)/1000:1/60,reduced.matches?.045:.10);lastFrame=time;camera.position.fromArray(shown.position);target.fromArray(shown.target);camera.lookAt(target);camera.updateMatrixWorld();
 ...
 stage.update(time,hover,selected,!(fireOverride??!reduced.matches),camera);const renderStart=performance.now();renderer.render(stage.scene,camera);renderMilliseconds+=performance.now()-renderStart;frames++;if(time-lastMeasure>1500){const stats=$('performance');stats.textContent=Math.round(frames*1000/(time-lastMeasure))+' FPS';Object.assign(stats.dataset,{renderSubmissionMs:(renderMilliseconds/frames).toFixed(2),calls:String(renderer.info.render.calls),triangles:String(renderer.info.render.triangles),textures:String(renderer.info.memory.textures),lighting:params.get('lighting')==='legacy'?'legacy':'hdr'});frames=0;renderMilliseconds=0;lastMeasure=time;}frame=requestAnimationFrame(animate);}
document.addEventListener('visibilitychange',()=>{cancelAnimationFrame(frame);motion.cancel();if(!document.hidden){frames=0;renderMilliseconds=0;lastMeasure=performance.now();frame=requestAnimationFrame(animate);}});
