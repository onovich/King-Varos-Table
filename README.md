...
- `web/` 是可直接运行的浏览器原型：左键标亮、右键或 Shift+点击标暗，提示器每次按当前盘面重算，优先给出使用当前落子的基础关系，并用证明卡片展示线索范围、差集和结论。
- 公开关卡 JSON 会同时记录关卡级 `reasoningLevel` 与区域级推理统计；“含高级推理”表示整条解题路径包含高级步骤，真正执行到该步骤时显示“高级推理”。
